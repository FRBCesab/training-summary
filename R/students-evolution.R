students_evolution <- function(inset = TRUE) {
  ...
  students <- tapply(y$"fullname", list(y$"year", y$"course"), length, default = 0) |> 
    as.data.frame()
  ...
  students <- students[ , c(1, 5, 6, 3, 4, 2)]
  ...
  filename <- ifelse(inset, "number_of_trainees-1.png", "number_of_trainees-0.png")
  
  png(filename  = here::here("figures", filename), 
      width     = 16, 
      height    = 8, 
      units     = "in", 
      pointsize = 16,
  ...
  plot(0, pch = 19, xlim = range(students$year),
       ylim = c(0, 260), xlab = "", cex = 1.3, lwd = 1.75,
  ...
  for (i in 2:ncol(students)) {
    ...
    if (i == 2) rows <- 1:nrow(students)
    if (i == 3) rows <- 4:nrow(students)
    if (i == 4) rows <- 4:nrow(students)
    if (i == 5) rows <- 6:nrow(students)
    if (i == 6) rows <- 6:nrow(students)
    ...
    polygon(x = c(x_val, rev(x_val)), 
            y = c(y_up, rev(y_bot)),
            border = "#ffffff", 
            col = color_scheme()[colnames(x)[i]], lwd = 2)
  ...
  points(years, students, type = "p", pch = 19,
         cex = 1.65, col = "#ffffff")
  ...
  mtext(text = "Cumulative number of trainees", side = 2, line = 2.2, cex = 1.4, font = 2)
  ...
  if (inset) {
    ...
    plot(0, type = "n", xlim = c(0, 150), ylim = c(0.5, 5.5), axes = FALSE, ann = FALSE)
    # box(which = "figure")
  
    axis(3, at = seq(0, 140, by = 20), lwd = 0, lwd.ticks = 0)
    ...
    abline(v = seq(0, 140, by = 20), col = "#ffffff", lty = 3, lwd = 0.25)
  }
